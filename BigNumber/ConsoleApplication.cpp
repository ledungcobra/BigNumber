...
std::string ConsoleApplication::ProcessQint(std::string inputString)
{
	...
	{
		if (inputString.find(UnaryOperator.at(i), 0) != std::string::npos)
		{
			OperatorType = 1;
			break;
		}
	...
	if (OperatorType == 0)
	{
		...
		{
			if (inputString.find(BinaryOperator.at(i), 0) != std::string::npos)
			{
				OperatorType = 2;
				break;
			}
...
void ConsoleApplication::Run(std::string fileIn, std::string fileOut, std::string type)
{
	...
	else
	{
		if (type == "1")
			...
			{
				fout << ProcessQint(list.at(i)) << "\n";
			}
		else
			for (unsigned int i = 0; i < list.size(); i++)
			{
				fout << ProcessQfloat(list.at(i)) << "\n";
			}
	}
